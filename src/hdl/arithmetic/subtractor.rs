...
pub fn goldilocks_sub_comb() -> Result<GoldilocksSubArrow, Error> {
    ...
    let bld = bld.with_instruction(Op::Bin(BinOp::Sub), vec![diff64, wrap_corr], diff_minus_corr)?;
...
mod tests {
    ...
    fn sub_underflow_correction() -> Result<(), Error> {
        ...
        let inputs: Vec<_> = test_cases.iter().map(|&(a, b)| {
            crate::hdl::common::u64_to_bitseq(a)
                .concat(crate::hdl::common::u64_to_bitseq(b))
        }).collect();
        ...
        test_cases.iter().zip(results.iter()).try_for_each(|(&(a, b), sample)| {
            let output_val = crate::hdl::common::bitseq_to_u64(sample.value())?;
